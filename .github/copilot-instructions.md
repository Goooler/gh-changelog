...
## Technology Stack
- **Language**: Go
- **CLI framework**: Cobra, packaged as a GitHub CLI (`gh`) extension
...
- **Constants**: Use camelCase for unexported constants and PascalCase for exported constants
...
- Use Cobra commands and flags for command-line parsing
- Add `github.com/cli/go-gh` only when the extension needs GitHub API access
- Follow the standard Go CLI pattern: `command -> subcommand -> flags`
- Preserve Cobra's automatic `--help` and the template's `--version` support
...
- Build: `go build -o gh-extension-template .` or use `make build`
